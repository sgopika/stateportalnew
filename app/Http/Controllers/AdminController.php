...
//                $number = $mobile;
    ...
    public function contenttypestatus(Request $request, $id)
    {
     ...
     /*    Content types   */   
      /*     Story Category   */ 
    public function storycategorylist(Request $request)
    {
           $listdata = DB::table('storycategories')
            ->select('id','name','malname','status')
            ->get();
        

        return view('admin.storycategorylist',compact('listdata'));
    }

    public function storycategorystore(Request $request)
    {
        $request->validate([
            'name'  =>'required|min:3|max:20|regex:/(^[-0-9A-Za-z\s ]+$)/',
            'malname' =>'required|min:3|max:50|regex:/(^[\P{Malayalam}_.,]+$)/'
        ...
        if($request->ajax())
        {
            $chkrows= Storycategory::where('name',$request->name)->exists() ? 1 : 0;
            if($chkrows==0){
                $resultsave = new Storycategory([
                    'name'        =>  $request->name,
    ...
    public function storycategoryedit(Request $request, $id)
    {
        if($request->ajax())
        {
            $resultdata = Storycategory::find($id);
            return response()->json(['resultdata' => $resultdata]);
        }

    }

    public function storycategoryupdate(Request $request)
    {
        $request->validate([
            'name'  =>'required|min:3|max:20|regex:/(^[-0-9A-Za-z\s ]+$)/',
            'malname'   =>'required|min:3|max:50|regex:/(^[\P{Malayalam}_.,]+$)/'
        ...
        if($request->ajax())
        {
            ...
            if($chkrows==0){
                ...
                Storycategory::whereId($request->hidden_id)->update($form_data);
                return response()->json(['success' => 'Data is successfully updated']);
            } else {
                return response()->json(['errors' => 'Already a Story Category with same name exists.']);
            }    
        }
        
    }

    public function storycategorydestroy(Request $request, $id)
    {
        if($request->ajax())
        {
            Storycategory::findOrFail($id)->delete();
        }
        return response()->json(['success' => 'Data is successfully updated']);
    }
    
    public function storycategorystatus(Request $request, $id)
    {
        if($request->ajax())
        {
            $getstatus = DB::table('storycategories')
                ->select('status')
                ->where('id',$id)
                ->first();
            ...
            if($status==1){
                $upd_status = array('status' => 2);
                DB::table('storycategories')->where('id',$id)->update($upd_status);
                 return response()->json(['success' => 'Data Updated successfully.']);
            }
            else{
    ...
    public function filetypestatus(Request $request, $id)
    {
     ...
     /*     Field Department    */ 
    public function fielddepartmentlist(Request $request)
    {
           $listdata = DB::table('fielddepartments')
           ->join('departments','departments.id','=','fielddepartments.departments_id')
           ->select('fielddepartments.id','fielddepartments.name','fielddepartments.malname','departments.entitle as title','fielddepartments.status')
            ->get();
        

        return view('admin.fielddepartmentlist',compact('listdata'));
    }


     public function fielddepartmentcreate(Request $request)
    {
        if($request->ajax())
        {
    ...
    public function fielddepartmentstore(Request $request)
    {
        $request->validate([
            'name'  =>'required|min:3|max:20|regex:/(^[-0-9A-Za-z\s ]+$)/',
            'malname'   =>'required|min:3|max:50|regex:/(^[\P{Malayalam}_.,]+$)/',
        ...
        if($request->ajax())
        {
            $chkrows= Fielddepartment::where('name',$request->name)->exists() ? 1 : 0;
            if($chkrows==0){
                $resultsave = new Fielddepartment([
                    'name'        =>  $request->name,
                    'malname'        =>  $request->malname,
                    'departments_id'        =>  $request->departmentsid,
                    'users_id'       => Auth::user()->id
                ]); 
                $resultsave->save();
                return response()->json(['success' => 'Data Added successfully.']);
            } else {
                return response()->json(['errors' => 'Already a Field Department with same name exists.']);
            }    
        }
        
    }

    public function fielddepartmentedit(Request $request, $id)
    {
        if($request->ajax())
        {
            $resultdata = Fielddepartment::find($id);
             $departments    = DB::table('departments')->orderBy('id','asc')->get();
            return response()->json(['resultdata' => $resultdata,'departments'=>$departments]);
        }

    }

    public function fielddepartmentupdate(Request $request)
    {
        $request->validate([
            'name'  =>'required|min:3|max:20|regex:/(^[-0-9A-Za-z\s ]+$)/',
            'malname'   =>'required|min:3|max:50|regex:/(^[\P{Malayalam}_.,]+$)/',
        ...
        if($request->ajax())
        {
            $chkrows= Fielddepartment::where('id','!=',$request->hidden_id)->where('name',$request->name)->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'name'        =>  $request->name,
                    'malname'        =>  $request->malname,
                ...
                Fielddepartment::whereId($request->hidden_id)->update($form_data);
                return response()->json(['success' => 'Data is successfully updated']);
            } else {
                return response()->json(['errors' => 'Already a field department with same name exists.']);
            }    
    ...
    public function fielddepartmentdestroy(Request $request, $id)
    {
        if($request->ajax())
        {
            Fielddepartment::findOrFail($id)->delete();
        }
        return response()->json(['success' => 'Data is successfully updated']);
    }
    
    public function fielddepartmentstatus(Request $request, $id)
    {
        if($request->ajax())
        {
            $getstatus = DB::table('fielddepartments')
                ->select('status')
                ->where('id',$id)
                ->first();
            ...
            if($status==1){
                $upd_status = array('status' => 2);
                DB::table('fielddepartments')->where('id',$id)->update($upd_status);
                 return response()->json(['success' => 'Data Updated successfully.']);
            }
            else{
    ...
    {
           $listdata = DB::table('documenttypes')
            ->select('id','name', 'malname', 'reviewdays', 'archivedays', 'cmap', 'documentcategories_id', 'status')
            ->get();
        

        return view('admin.documenttypelist',compact('listdata'));
    }
     public function documenttypecreate(Request $request)
    {
        if($request->ajax())
        {
              
            $documentcategories  = DB::table('documentcategories')->orderBy('id','asc')->get();
    ...
    public function documenttypestore(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Documenttype::where('name',$request->name)->exists() ? 1 : 0;
            if($chkrows==0){
                $resultsave = new Documenttype([
                    'name'                 =>  $request->name,
                    'malname'              =>  $request->malname,
                    'reviewdays'           =>  $request->reviewdays,
                    'archivedays'          =>  $request->archivedays,
                    'cmap'                 =>  $request->cmap,
                    'documentcategories_id'=>  $request->documentcategories,
                    'users_id'       => Auth::user()->id
                ]); 
                $resultsave->save();
                return response()->json(['success' => 'Data Added successfully.']);
            } else {
                return response()->json(['errors' => 'Already an Document Type with same name exists.']);
            }    
        }
    ...
    public function documenttypeedit(Request $request, $id)
    {
        if($request->ajax())
        {
            $resultdata = Documenttype::find($id);
             $documentcategories    = DB::table('documentcategories')->orderBy('id','asc')->get();
            return response()->json(['resultdata' => $resultdata,'documentcategories'=>$documentcategories]);
        }
    ...
    public function documenttypeupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Documenttype::where('name',$request->name)->where('id','!=',$request->hidden_id)->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'name'                 =>  $request->name,
                    'malname'              =>  $request->malname,
                ...
                Documenttype::whereId($request->hidden_id)->update($form_data);
                return response()->json(['success' => 'Data is successfully updated']);
            } else {
                return response()->json(['errors' => 'Already a Documenttype with same name exists.']);
            }    
        }
        
    }

    public function documenttypedestroy(Request $request, $id)
    {
        if($request->ajax())
        {
    ...
    public function documenttypestatus(Request $request, $id)
    {
        if($request->ajax())
        {
            $getstatus = DB::table('documenttypes')
                ->select('status')
                ->where('id',$id)
                ->first();
            ...
            if($status==1){
                $upd_status = array('status' => 2);
                DB::table('documenttypes')->where('id',$id)->update($upd_status);
                 return response()->json(['success' => 'Data Updated successfully.']);
            }
            else{
     ...
     public function usercreate(Request $request)
    {
        if($request->ajax())
        {
            ...
            return response()->json(['department' => $department,'designation'=>$designation,'office'=>$office,'staffcategory'=>$staffcategory,'usertype'=>$usertype, 'district'=>$district]);
        }
    }
    ...
    public function useredit(Request $request, $id)
    {
        if($request->ajax())
        {
            ...
            $staffcategory   = DB::table('staffcategories')->orderBy('id','asc')->get();
            $usertype    = DB::table('usertypes')->orderBy('id','asc')->get();
            $fielddepartment=DB::table('fielddepartments')->orderBy('id','asc')->get();
            $district    = DB::table('districts')->orderBy('id','asc')->get();
     return response()->json(['resultdata' => $resultdata,'department' => $department,'designation'=>$designation,'office'=>$office,'staffcategory'=>$staffcategory,'usertype'=>$usertype,'fielddepartment'=>$fielddepartment,'district'=>$district]);
    ...
    public function userupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= User::where('id','!=',$request->hidden_id)->where('name',$request->name)->exists() ? 1 : 0;
            if($chkrows==0){
                if($request->usertypeid==10){
    ...
    /*     Users    */
     ...
     /////////////////////User Module  Permission(start)////////////////////////////////////
    
    
    public function usermodulepermissionlist(Request $request)
    {

       
           $listdata = DB::table('usermodules')
            ->join('usertypes','usertypes.id','=','usermodules.usertypes_id')
            ->select('usermodules.id as id','usermodules.documenttypes_id as componenttitle','usertypes.entitle as usertypes_id','usermodules.status as status')
            ->get();
         //$usertype     = DB::table('usertypes')->orderBy('id','asc')->get();
         $documenttype     = DB::table('documenttypes')->orderBy('id','asc')->get();


        return view('admin.usermodulepermissionlist',compact('listdata','documenttype'));
    }

    public function usermodulepermissioncreate(Request $request)
    {
        if($request->ajax())
        {
            ...
            return response()->json(['usertype' => $usertype, 'documenttype' => $documenttype]);
        }
    }


    public function usermodulepermissionstore(Request $request)
    {
        $request->validate([
            'documentid'  =>'required',
            'usertypeid'  =>'required'
        ...
        if($request->ajax())
        {
            $documentid=implode(',', $request->documentid);

            $userid=Auth::user()->id;
            $chkrows= Usermodule::where('usertypes_id',$request->usertypeid)->exists() ? 1 : 0;
            if($chkrows==0){



                $resultsave = new Usermodule([
                    'usertypes_id'   =>  $request->usertypeid,
                    'documenttypes_id'    =>  $documentid,
                    'enteredusers_id'    =>   $userid
                ]); 
                $resultsave->save();
                return response()->json(['success' => 'Data Added successfully.']);
            } else {
   ...
   public function usermodulepermissionedit(Request $request, $id)
    {
        if($request->ajax())
        {
            $resultdata = Usermodule::find($id);
            $usertype     = DB::table('usertypes')->orderBy('id','asc')->get();
            $documenttype    = DB::table('documenttypes')->orderBy('id','asc')->get();
            return response()->json(['resultdata' => $resultdata,'usertype' => $usertype, 'documenttype' => $documenttype]);
        }

    }

    public function usermodulepermissionupdate(Request $request)
    {
          $request->validate([
            'documentid'  =>'required',
            'usertypeid'  =>'required'
        ...
        $chkrows= Usermodule::where('usertypes_id',$request->usertypeid)->where('id','!=',$request->hidden_id)->exists() ? 1 : 0;
            ...
            if($chkrows==0){
                ...
                Usermodule::whereId($request->hidden_id)->update($form_data);
                return response()->json(['success' => 'Data is successfully updated']);
            } else {
                return response()->json(['errors' => 'Already a permission set  with same user type exists.']);
            }    
    ...
    public function usermodulepermissiondestroy(Request $request, $id)
    {
        if($request->ajax())
        {
            Usermodule::findOrFail($id)->delete();
        }
        return response()->json(['success' => 'Data is successfully updated']);
    }
    
    public function usermodulepermissionstatus(Request $request, $id)
    {
        if($request->ajax())
        {
            $getstatus = DB::table('usermodules')
                ->select('status')
                ->where('id',$id)
                ->first();
            ...
            if($status==1){
                $upd_status = array('status' => 2);
                DB::table('usermodules')->where('id',$id)->update($upd_status);
                 return response()->json(['success' => 'Data Updated successfully.']);
            }
            else{
...
public function languagelist(Request $request)
    ...
    {
       $request->validate([
            'name'  =>'required|min:3|max:800|regex:/(^[A-Za-z,.&\s ]+$)/',
            'malname'   =>'required|min:3|max:1000|regex:/(^[\P{Malayalam}_.,&]+$)/'
    ...
    public function sitecontrollabelupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Sitecontrollabel::where('id','!=',$request->hidden_id)->where('entitle',$request->name)->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'    =>  $request->name,
                    'maltitle'   =>  $request->malname,
 ...
 /* Key Word  */
    ...
    {
           $listdata = DB::table('keywords')
            ->select('id','entitle','maltitle','status')
            ->get();
        

        return view('admin.keywordlist',compact('listdata'));
    }
    ...
    public function keywordstore(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Keyword::where('entitle',$request->entitle)->exists() ? 1 : 0;
            if($chkrows==0){
                $resultsave = new Keyword([
                    'entitle'        =>  $request->entitle,
                    'maltitle'        =>  $request->maltitle,
                    //'users_id'        =>  Auth::user()->id
                ]); 
    ...
    public function keywordedit(Request $request, $id)
    {
        if($request->ajax())
        {
            $resultdata = Keyword::find($id);
            return response()->json(['resultdata' => $resultdata]);
        }
    ...
    public function keywordupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Keyword::where('id','!=',$request->hidden_id)->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'        =>  $request->entitle,
                    'maltitle'        =>  $request->maltitle,
                ...
                Keyword::whereId($request->hidden_id)->update($form_data);
                return response()->json(['success' => 'Data is successfully updated']);
            } else {
                return response()->json(['errors' => 'Already a Department Category with same name exists.']);
            }    
    ...
    public function keyworddestroy(Request $request, $id)
    {
        if($request->ajax())
        {
    ...
    public function keywordstatus(Request $request, $id)
    {
        if($request->ajax())
        {
            $getstatus = DB::table('keywords')
                ->select('status')
                ->where('id',$id)
                ->first();
            ...
            if($status==1){
                $upd_status = array('status' => 2);
                DB::table('keywords')->where('id',$id)->update($upd_status);
                 return response()->json(['success' => 'Data Updated successfully.']);
            }
            else{
