    ...
    */
    ...
    public function checklogin(Request $request){
        ...
        if(Auth::attempt($user_data)){
            ...
            return redirect('/documentmoderator');
            }
             elseif (Auth::user()->usertypes_id == 5) {
            return redirect('/documentapprover');
            }
           elseif (Auth::user()->usertypes_id == 7) {
            return redirect('/storycontributor');
            }
            elseif (Auth::user()->usertypes_id == 8) {
            return redirect('/storymoderator');
            }
            elseif (Auth::user()->usertypes_id == 9) {
            return redirect('/storyapprover');
            }
            elseif (Auth::user()->usertypes_id == 10) {
            return redirect('/districtusr');
            }
           /*  elseif (Auth::user()->usertypes_id == 5) {
