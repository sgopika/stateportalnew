...
<div class="row ">
  ...
  @endif
    ...
    <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;  Field Department List </strong></p>
   </div>
      ...
      <div class="modal-body adminpage">
        ...
        <div id="form_section">
          ...
          <div class="row customformrow">
            ...
            <div class="col-md-6 py-2">
               <select class="form-control customform eng_xxxs fg-darkCrimson" id="departmentsid" name="departmentsid">
...
$("#transactionbutton").click(function(event){
    ...
    $("#transactionmodal").modal('show');
     ...
     $.ajax({
       url: "{{ route('admin.fielddepartmentcreate') }}",
       dataType:"json",
       success:function(data)
        {
             $('#departmentsid').empty();
            $('#departmentsid').append($('<option></option>').val('0').html('Select'));
            $.each(data.departments, function(index, element) {
                $('#departmentsid').append(
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('admin.fielddepartmentstore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('admin.fielddepartmentupdate') }}";
  ...
  $(document).on('click', '.edit', function(){
      ...
      $('#ajaxformresults').html('');
      $.ajax({
       url :"/admin/fielddepartmentedit/"+id,
       dataType:"json",
       success:function(data)
        { 
           ...
           $('#departmentsid').empty();
            ...
            $.each(data.departments, function(index, element) {
                $('#departmentsid').append(
                 ...
                 element.id == data.resultdata.departments_id ? $('#departmentsid').val(element.id).prop('selected', true) : '';
            });
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
    $.ajax({
      url:"/admin/fielddepartmentstatus/"+id,
      dataType:"json",
  ...
  $('#ok_button').click(function(){
    ...
    $.ajax({
            url:"/admin/fielddepartmentdestroy/"+element_id,
            dataType:"json",
