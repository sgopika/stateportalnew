...
<div class="row ">
  ...
  @endif
    ...
    <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;  Document Type List </strong></p>
   </div>
            ...
            <tbody class="eng_xxxs">
               @php
                ...
                <tr>
                    ...
                    <td><span class="eng_xxxxs"> {{ $res->malname }}</span> </td>
                    <td><span class="eng_xxxxs"> {{ $res->reviewdays }}</span> </td>
                    <td><span class="eng_xxxxs"> {{ $res->archivedays }}</span> </td>
                    <td><span class="eng_xxxxs"> @if($res->cmap==7) 3 tiered  @elseif($res->cmap==6) 2 tiered @elseif($res->cmap==4) 1 tiered @endif</span> </td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body adminpage">
        ...
        <div id="form_section">
          ...
          <div class="row customformrow">
            <div class="col-md-6 py-2">
              <label for="IDNAME" class="eng_xxxs fg-darkBrown">Review Days</label>
              
            </div> <!-- ./col-md-6 -->
            <div class="col-md-6 py-2">
                <input type="text" class="form-control customform eng_xxxs fg-darkCrimson" id="reviewdays" name="reviewdays" aria-describedby="HELPNAME" placeholder="Review Days" maxlength="3" minlength="1">
                <p id="reviewdayserror" style="display:none; color:#FF0000;" >Only numbers are allowed.</p>
            </div> <!-- ./col-md-6 -->
          </div> <!-- ./row -->
          <div class="row customformrow">
            <div class="col-md-6 py-2">
              <label for="IDNAME" class="eng_xxxs fg-darkBrown">Archive Days</label>
              
            </div> <!-- ./col-md-6 -->
            <div class="col-md-6 py-2">
                <input type="text" class="form-control customform eng_xxxs fg-darkCrimson" id="archivedays" name="archivedays" aria-describedby="HELPNAME" placeholder="Archive Days" maxlength="3" minlength="1">
                <p id="archivedayserror" style="display:none; color:#FF0000;" >Only numbers are allowed.</p>
            </div> <!-- ./col-md-6 -->
          </div> <!-- ./row -->
          <div class="row customformrow">
            <div class="col-md-6 py-2">
              <label for="IDNAME" class="eng_xxxs fg-darkBrown">CMAP</label>
              
            </div> <!-- ./col-md-6 -->
            <div class="col-md-6 py-2">
               <select class="form-control customform eng_xxxs fg-darkCrimson" id="cmap" name="cmap">
            <option>Select CMAP</option>
            <option value="7">3 tiered</option>
            <option value="6">2 tiered</option>
            <option value="4">1 tiered</option>
          ...
          <div class="row customformrow">
            <div class="col-md-6 py-2">
              <label for="IDNAME" class="eng_xxxs fg-darkBrown">Document Categories</label>
              
            </div> <!-- ./col-md-6 -->
            <div class="col-md-6 py-2">
               <select class="form-control customform eng_xxxs fg-darkCrimson" id="documentcategories" name="documentcategories">
...
$('#resposivetable').DataTable( {
 ...
 $('#malname').on('change ', function(e) {
  ...
  var pattern = new XRegExp("^[\\p{InMalayalam} 0-9_.,]+$");
...
$('#malname').val('');
 ...
 $('#reviewdays').on('change ', function(e) {
  var testval = this.value;
  var tested = new RegExp('^[a-zA-Z0-9 \s]+$');
  if (!tested.test(testval))
  {
    $('#reviewdays').val('');
     ...
     $('#reviewdayserror').slideDown("slow");

  }
  else
  {
     $('#reviewdayserror').hide();
     
  }
      
});
 $('#archivedays').on('change ', function(e) {
  var testval = this.value;
  var tested = new RegExp('^[a-zA-Z0-9 \s]+$');
  if (!tested.test(testval))
  {
    $('#archivedays').val('');
     ...
     $('#archivedayserror').slideDown("slow");

  }
  else
  {
     $('#archivedayserror').hide();
...
$("#transactionbutton").click(function(event){
    ...
    $("#transactionmodal").modal('show');
     ...
     $.ajax({
       url: "{{ route('admin.documenttypecreate') }}",
       dataType:"json",
       success:function(data)
        {
            $('#documentcategories').empty();
            $('#documentcategories').append($('<option></option>').val('0').html('Select'));
            $.each(data.documentcategories, function(index, element) {
                $('#documentcategories').append(
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('admin.documenttypestore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('admin.documenttypeupdate') }}";
    ...
    $.ajax({
         ...
         { 
            ...
            if(data.errors)
            {
               alert("Already a file type with same name exists");
  ...
  $(document).on('click', '.edit', function(){
      ...
      $.ajax({
       ...
       success:function(data)
        { 
          ...
          $('#reviewdays').val(data.resultdata.reviewdays);
          $('#archivedays').val(data.resultdata.archivedays);
          
          $("#cmap option").each(function(){
              if($(this).val()==data.resultdata.cmap){
           ...
           $('#documentcategories').empty();
            ...
            $.each(data.documentcategories, function(index, element) {
                $('#documentcategories').append(
                 ...
                 element.id == data.resultdata.documentcategories_id ? $('#documentcategories').val(element.id).prop('selected', true) : '';
            });
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
    $.ajax({
      url:"/admin/documenttypestatus/"+id,
      dataType:"json",
  ...
  $('#ok_button').click(function(){
    ...
    $.ajax({
            url:"/admin/documenttypedestroy/"+element_id,
            dataType:"json",
