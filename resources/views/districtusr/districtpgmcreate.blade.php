...
<div class="container-fluid homepage adminpage">
    <div class="row ">
        ...
        <div class="col-12 py-1 px-2 ">
            <p class="py-2"> <strong> <i class="fas fa-hand-point-right"></i> &nbsp; Add New District Programme</strong></p>
        </div> <!-- ./col12 -->
    ...
    @if(Auth::user()->usertypes_id==10)
    <form id="ajaxmodalform" action="{{ route('districtusr.districtpgmstore') }}" method="post" class="form-horizontal ajaxmodalform" enctype="multipart/form-data">
...
<script>
  ...
  content_style: 'body { font-family:Helvetica,Arial,sans-serif; font-size:14px }'
        ...
        $('#upldfile').bind('change', function() {
            ...
            if ($.inArray(ext, ['png', 'jpg', 'jpeg']) == -1) {
                $('#imageerrorr').slideDown("slow");
                $('#imageerrorr1').slideUp("slow");
                $('#upldfile').val('');

            } else {
                var picsize = (this.files[0].size);
                if (picsize > 5000000) {
                    $('#imageerrorr1').slideDown("slow");
                    $('#upldfile').val('');
