...
<div class="row ">
  ...
  <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;General report</strong></p>
   </div>
               ...
               @php
                ...
                <tr>
                    ...
                    <td><span class="eng_xxxxs"> {{ date('d-m-Y', strtotime($res->documentdate)) }} </span> </td> 
                     ...
                     <td><span class="eng_xxxxs"> <a target="_blank" href="{{ asset('Generalreport').'/'.$res->filepath }}">View File</a></span> </td>
...
$("#transactionbutton").click(function(event){
    ...
    $('.modal-title').text('Add New General report');
  ...
  var action_url = '';
 action_url="{{ route('contributor.generalreportcreate') }}";
...
$('#ajaxmodalform').on('submit', function(event){ 
   var formData = new FormData(this);
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('contributor.generalreportstore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('contributor.generalreportupdate') }}";
    ...
    $.ajax({
         ...
         { 
            ...
            if(data.errors)
            {
               alert("Already a general report with same exists");
  ...
  $(document).on('click', '.edit', function(){
      ...
      var action_url2 = '';
         ...
         action_url2 = "/contributor/generalreportedit/"+id;
        ...
        { 
          if(data.error){
            alert("The Article is Locked, So cannot be edited!");
             ...
             var urlpath="{{asset('Generalreport')}}/"+data.resultdata.filepath;
               $('#filepdf').show();
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
      ...
      var action_url3 = '';
         ...
         action_url3 = "/contributor/generalreportstatus/"+id;
  ...
  $('#ok_button').click(function(){
    var usertype=$("#usertypeid").val(); 
      var action_url4 = '';
         ...
         action_url4 = "/contributor/generalreportdestroy/"+element_id;
