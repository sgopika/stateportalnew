...
<div class="row ">
  ...
  <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;Notification</strong></p>
   </div>
               ...
               @php
                ...
                <tr>
                    ...
                    <td><span class="eng_xxxxs"> {{ date('d-m-Y', strtotime($res->documentdate)) }} </span> </td> 
                     ...
                     <td><span class="eng_xxxxs"> <a target="_blank" href="{{ asset('Notification').'/'.$res->filepath }}">View File</a></span> </td>
...
$("#transactionbutton").click(function(event){
    ...
    $('.modal-title').text('Add New Notification');
  ...
  var action_url = '';
 action_url="{{ route('contributor.notificationcreate') }}";
...
$('#ajaxmodalform').on('submit', function(event){ 
   var formData = new FormData(this);
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('contributor.notificationstore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('contributor.notificationupdate') }}";
    ...
    $.ajax({
         ...
         { 
            ...
            if(data.errors)
            {
               alert("Already a Notification with same exists");
  ...
  $(document).on('click', '.edit', function(){
      ...
      var action_url2 = '';
         ...
         action_url2 = "/contributor/notificationedit/"+id;
        ...
        { 
          if(data.error){
            alert("The Article is Locked, So cannot be edited!");
             ...
             var urlpath="{{asset('Notification')}}/"+data.resultdata.filepath;
             $('#filepdf').show();
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
      ...
      var action_url3 = '';
         ...
         action_url3 = "/contributor/notificationstatus/"+id;
  ...
  $('#ok_button').click(function(){
    var usertype=$("#usertypeid").val(); 
      var action_url4 = '';
         ...
         action_url4 = "/contributor/notificationdestroy/"+element_id;
