...
<div class="row ">
  ...
  <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;Disciplinary Action</strong></p>
   </div>
               ...
               @php
                ...
                <tr>
                    ...
                    <td><span class="eng_xxxxs"> {{ date('d-m-Y', strtotime($res->documentdate)) }} </span> </td> 
                     ...
                     <td><span class="eng_xxxxs"> <a target="_blank" href="{{ asset('Disciplinary').'/'.$res->filepath }}">View File</a></span> </td>
...
$("#transactionbutton").click(function(event){
    ...
    $('.modal-title').text('Add New Disciplinary action');
  ...
  var action_url = '';
 action_url="{{ route('contributor.disciplinarycreate') }}";
...
$('#ajaxmodalform').on('submit', function(event){ 
   var formData = new FormData(this);
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('contributor.disciplinarystore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('contributor.disciplinaryupdate') }}";
    ...
    $.ajax({
         ...
         { 
            ...
            if(data.errors)
            {
               alert("Already a disciplinary action with same exists");
  ...
  $(document).on('click', '.edit', function(){
      ...
      var action_url2 = '';
         ...
         action_url2 = "/contributor/disciplinaryedit/"+id;
        ...
        { 
          if(data.error){
            alert("The Article is Locked, So cannot be edited!");
             ...
             var urlpath="{{asset('Disciplinary')}}/"+data.resultdata.filepath;
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
      ...
      var action_url3 = '';
         ...
         action_url3 = "/contributor/disciplinarystatus/"+id;
  ...
  $('#ok_button').click(function(){
    var usertype=$("#usertypeid").val(); 
      var action_url4 = '';
         ...
         action_url4 = "/contributor/disciplinarydestroy/"+element_id;
