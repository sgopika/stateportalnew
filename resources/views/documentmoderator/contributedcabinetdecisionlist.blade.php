                ...
                <tr>
                    ...
                    <td><span class="eng_xxxxs">
                      ...
                      @if($documenttypes->id==$res->documenttypes_id)
                        @if($documenttypes->id=='2')
                        <a target="_blank" href="{{ asset('Cabinetdecision').'/'.$res->filepath }}">View File</a>
                        @else
                      <img src="{{ asset('Cabinetdecision').'/'.$res->filepath }}" class='img-thumbnail displaythumbnail customform eng_xxxs fg-darkCrimson' alt='Image' id='uploadedimage' width='150px'>
                      @endif
                      @endif
                      @endforeach
...
$(document).on('click', '.verify', function(){
      ...
      var action_url = '';
      if(usertype==4){
         action_url = "/documentmoderator/contributedcabinetdecisionverify/"+id;
      } else if(usertype==5){
         action_url = "/documentapprover/contributedcabinetdecisionverify/"+id;
      }
      $.ajax({
       url :action_url,
       dataType:"json",
...
$(document).on('click', '.reject', function(){
      ...
      var action_url1 = '';
      if(usertype==4){
         action_url1 = "/documentmoderator/contributedcabinetdecisionverify/"+id;
      } else if(usertype==5){
         action_url1 = "/documentapprover/contributedcabinetdecisionverify/"+id;
      }
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var usertype=$("#usertypeid").val(); 
      var action_url2 = '';
      if(usertype==4){
         action_url2 = "{{ route('documentmoderator.contributedcabinetdecisionupdate') }}";
      } else if(usertype==5){
         action_url2 = "{{ route('documentapprover.contributedcabinetdecisionupdate') }}";
      }
