                ...
                <tr>
                    ...
                    <td><span class="eng_xxxxs">
                      ...
                      @if($documenttypes->id==$res->documenttypes_id)
                        ...
                        <a target="_blank" href="{{ asset('Circulars').'/'.$res->filepath }}">View File</a>
                        @else
                      <img src="{{ asset('Circulars').'/'.$res->filepath }}" class='img-thumbnail displaythumbnail customform eng_xxxs fg-darkCrimson' alt='Image' id='uploadedimage' width='150px'>
                      @endif
                      @endif
                      @endforeach
...
$(document).on('click', '.verify', function(){
      ...
      var action_url = '';
      if(usertype==4){
         action_url = "/documentmoderator/circularverify/"+id;
      } else if(usertype==5){
         action_url = "/documentapprover/circularverify/"+id;
      }
      $.ajax({
       url :action_url,
       dataType:"json",
...
$(document).on('click', '.reject', function(){
      ...
      var action_url1 = '';
      if(usertype==4){
         action_url1 = "/documentmoderator/circularverify/"+id;
      } else if(usertype==5){
         action_url1 = "/documentapprover/circularverify/"+id;
      }
...
$('#ajaxmodalform').on('submit', function(event){ 
    event.preventDefault();
     var usertype=$("#usertypeid").val(); 
      var action_url2 = '';
      if(usertype==4){
         action_url2 = "{{ route('documentmoderator.circularupdate') }}";
      } else if(usertype==5){
         action_url2 = "{{ route('documentapprover.circularupdate') }}";
      }
