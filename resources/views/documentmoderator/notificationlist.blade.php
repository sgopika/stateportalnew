...
<div class="row ">
 ...
 <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;Notification list</strong></p>
   </div>
                ...
                <tr>
                    ...
                    <td><span class="eng_xxxxs">
                      ...
                      @if($documenttypes->id==$res->documenttypes_id)
                        ...
                        <a target="_blank" href="{{ asset('Notification').'/'.$res->filepath }}">View File</a>
                        @else
                      <img src="{{ asset('Notification').'/'.$res->filepath }}" class='img-thumbnail displaythumbnail customform eng_xxxs fg-darkCrimson' alt='Image' id='uploadedimage' width='150px'>
                      @endif
                      @endif
                      @endforeach
...
$(document).on('click', '.verify', function(){
      ...
      var action_url = '';
      if(usertype==4){
         action_url = "/documentmoderator/notificationverify/"+id;
      } else if(usertype==5){
         action_url = "/documentapprover/notificationverify/"+id;
      }
      $.ajax({
       url :action_url,
       dataType:"json",
...
$(document).on('click', '.reject', function(){
      ...
      var action_url1 = '';
      if(usertype==4){
         action_url1 = "/documentmoderator/notificationverify/"+id;
      } else if(usertype==5){
         action_url1 = "/documentapprover/notificationverify/"+id;
      }
      $.ajax({
       url :action_url1,
       dataType:"json",
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var usertype=$("#usertypeid").val(); 
      var action_url2 = '';
      if(usertype==4){
         action_url2 = "{{ route('documentmoderator.notificationupdate') }}";
      } else if(usertype==5){
         action_url2 = "{{ route('documentapprover.notificationupdate') }}";
      }
