...
<div class="row ">
 ...
 <div class="col-12 py-1">
  ...
  @endif
    ...
    <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;  Site Setting List </strong></p>
   </div>
  <div class="col-12 py-1">
...
$("#transactionbutton").click(function(event){
    ...
    $('#nameerror').hide();
    $('.modal-title').text('Add New Site setting');
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('admin.sitesettingstore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('admin.sitesettingupdate') }}";
              ...
              swal({
                ...
                .then((value) => {
                    if (value) {
                      ...
                      window.location.replace("{{route('admin.sitesettinglist')}}");
                    }
                });;
  ...
  $(document).on('click', '.edit', function(){
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
    $.ajax({
      url:"/admin/sitesettingstatus/"+id,
      dataType:"json",
  ...
  $(document).on('click', '.delete', function(){
      ...
      swal(
            {
              title: "Deleting Site Setting List Category",
              text: "Are you sure to delete Site Setting List Category?",
              type: "warning",
              buttons: true,
              dangerMode: true
            })
          .then((isconfirm) => {
            ...
            if (isconfirm) {
              $.ajax({
                  url:"/admin/sitesettingdestroy/"+element_id,
                  dataType:"json",
                  ...
                  success:function(data)
                  {
                    ...
                    $('#loader').hide();
                      ...
                      swal({
                                                        title:'Site Setting List Deleted',
                                                        text:data.success,
                                                        type:'info',                                               
                                                        buttonsStyling:false,
                                                        reverseButtons:true
