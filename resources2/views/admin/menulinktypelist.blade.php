...
<div class="row ">
 ...
 <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;  Menulink Type List </strong></p>
   </div>
  <div class="col-12 py-1">
  ...
  <div class="modal-dialog modal-lg" role="document">
    ...
    <div class="row customformrow">
      <div class="col-md-6 py-2">
        ...
        <small id="HELPNAME" class="form-text eng_xxxxs text-muted"> Menulink type name.</small>
      </div> <!-- ./col-md-6 -->
      <div class="col-md-6 py-2">
          <input required minlength="3" maxlength="50" type="text" class="form-control customform eng_xxxs fg-darkCrimson" id="name" name="name" aria-describedby="HELPNAME" placeholder="Placeholder value">
...
$("#transactionbutton").click(function(event){
    ...
    $('#nameerror').hide();
    $('.modal-title').text('Add New MenulinkType');
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('admin.menulinktypestore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('admin.menulinktypeupdate') }}";
    ...
    $.ajax({
         ...
         { 
            ...
            if(data.errors)
            {
              ...
              //  alert("Already an menulink type with same name exists");
              ...
              swal({
                ...
                .then((value) => {
                    if (value) {
                      ...
                      window.location.replace("{{route('admin.menulinktypelist')}}");
                    }
                });;
  ...
  $(document).on('click', '.edit', function(){
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
    $.ajax({
      url:"/admin/menulinktypestatus/"+id,
      dataType:"json",
  ...
  $(document).on('click', '.delete', function(){
      ...
      swal(
            {
              title: "Deleting Menulink Type List Category",
              text: "Are you sure to delete Menulink Type List Category?",
              type: "warning",
              buttons: true,
              dangerMode: true
            })
          .then((isconfirm) => {
            ...
            if (isconfirm) {
              $.ajax({
                  url:"/admin/menulinktypedestroy/"+element_id,
                  dataType:"json",
                  ...
                  success:function(data)
                  {
                    ...
                    $('#loader').hide();
                      ...
                      swal({
                                                        title:'Menulink Type List Deleted',
                                                        text:data.success,
                                                        type:'info',                                               
                                                        buttonsStyling:false,
                                                        reverseButtons:true
