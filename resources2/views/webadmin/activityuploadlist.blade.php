            ...
            <tbody class="eng_xxxs">
               @php
                ...
                <tr>
                    ...
                    @php
                      $ext = explode (".", $res->file);
                   
                    @endphp
                    @if ($ext[1]=='jpg')
                        <img src="{{asset('Activityupload').'/'.$res->file}}" class="img-thumbnail displaythumbnail" alt="Logo" height="200" width="200"> 
                    @endif
                    @if ($ext[1]=='mp3')
                      <audio controls autoplay>
                        <source src="{{asset('Activityupload').'/'.$res->file}}" type="audio/ogg">
                        <source src="{{asset('Activityupload').'/'.$res->file}}" type="audio/mpeg">
...
/*$('#ajaxmodalform').on('submit', function(event){ 
      ...
      $.ajax({
       ...
       success:function(data)
        { 
          ...
          var filshs='';
          var pathsurr="{{asset('Activityupload')}}/";
          if(ext[1]=='jpg'){
            $('#upfile').html('');
            $("#uploadedimage").show();
  ...
  $(document).on('click', '.delete', function(){
      ...
      swal(
            {
              title: "Deleting Activity Upload List",
              text: "Are you sure to delete Activity Upload List?",
              type: "warning",
              buttons: true,
              dangerMode: true
            })
            ...
            if (isconfirm) {
              $.ajax({
                  ...
                  success:function(data)
                  {
                    $('#loader').hide();
                      swal({
                                                        title:'Activity Upload List Deleted',
                                                        text:data.success,
                                                        type:'info',                                               
