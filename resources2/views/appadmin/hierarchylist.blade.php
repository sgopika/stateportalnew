...
<div class="row ">
 ...
 <div class="col-12 py-1 px-2 ">
    <p class="eng_xxs px-3 fg-darkBrown"> Hierarchy List </p>
  </div> <!-- ./col12 -->
...
$('#nameerror').hide();
    ...
    $('.modal-title').text('Add New Hierarchy');
...
$('#nameerror').hide();
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('appadmin.hierarchystore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('appadmin.hierarchyupdate') }}";
              ...
              swal({
                ...
                .then((value) => {
                    if (value) {
                      ...
                      window.location.replace("{{route('appadmin.hierarchylist')}}");
                    }
                });
...
$('#nameerror').hide();
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
    $.ajax({
      url:"/appadmin/hierarchystatus/"+id,
      dataType:"json",
  ...
  $(document).on('click', '.delete', function(){
      ...
      swal(
            {
              title: "Deleting Hierarchy  List",
              text: "Are you sure to delete Hierarchy  List?",
              type: "warning",
              buttons: true,
              dangerMode: true
            })
          .then((isconfirm) => {
            ...
            if (isconfirm) {
              $.ajax({
                  url:"/appadmin/hierarchydestroy/"+element_id,
                  dataType:"json",
