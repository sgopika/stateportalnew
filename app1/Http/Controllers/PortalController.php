...
class PortalController extends Controller
{
    ...
    public function articledetail(Request $request, $encid)
    {
        ...
        if ($sessionbil == 1) {
            ...
            $relatedarticles = DB::table('articles')->where('status', 1)->select('id', 'entitle as title', 'poster')->orderBy('id', 'desc')->limit(20)->get();
        } else {
            ...
            $relatedarticles = DB::table('articles')->where('status', 1)->select('id', 'maltitle as title', 'poster')->orderBy('id', 'desc')->limit(20)->get();
        }
    ...
    public function whatsnewdetail(Request $request)
    {
        $sessionbil = Session::get('bilingual');
        if ($sessionbil == 1) {
            $whatsnew = DB::table('componentarticles')->where('components_id',3)->where('status', 1)->select('id', 'entitle as title', 'ensubtitle as subtitle', 'encontent as content', 'iconclass')->orderBy('id', 'asc')->limit(3)->get();
        } else {
            $whatsnew = DB::table('componentarticles')->where('components_id',3)->where('status', 1)->select('id', 'maltitle as title', 'malsubtitle as subtitle', 'malcontent as content', 'iconclass')->orderBy('id', 'asc')->limit(3)->get();
        }
        return view('whatsnewdetail', compact('whatsnew', 'sessionbil'));
    }
        ...
        if ($sessionbil == 1) {
            ...
            $searchResults = (new Search())
                ...
                ->registerModel(Appdepartment::class, function (ModelSearchAspect $modelSearchAspect) {
                    $modelSearchAspect
                        ->addSearchableAttribute('enabout') // return results for partial matches on usernames
                        ->where('approve_status', 2);
                })
            ...
            $searchResults = (new Search())
                ->registerModel(Article::class, function (ModelSearchAspect $modelSearchAspect) {
                    $modelSearchAspect
                        ->addSearchableAttribute('malcontent') // return results for partial matches on usernames
                        ->where('approve_status', 2);
                })
                ->registerModel(Appdepartment::class, function (ModelSearchAspect $modelSearchAspect) {
                    $modelSearchAspect
