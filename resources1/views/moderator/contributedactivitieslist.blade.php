            ...
            <tr>
              ...
              @if($val==1)<td>
                <div class="btn-group" role="group" aria-label="Actionbuttons">
                  ...
                  <button type="button" class="edit btn btn-sm bg-lightBrown fg-darkCrimson eng_xxxxs reject" name="reject" id="{{ $res->id }}" data-toggle="tooltip" data-placement="top" title="Reject"> <i class="fas fa-reply"></i>&nbsp;Reject</button>
        ...
        <div class="modal-body adminpage">
          ...
          <div id="form_section">
            <div class="row customformrow">
              ...
              <div class="col-md-12 py-2">
                <textarea class="form-control customform eng_xxxs fg-darkCrimson" id="moderator_remarks" name="moderator_remarks" aria-describedby="HELPNAME" placeholder="Remarks.." required minlength="3" maxlength="1000"></textarea>
              </div> <!-- ./col-md-6 -->
              <p id="moderator_remarkserror" style="display:none; color:#FF0000;">Only Alphabets,dot and , are allowed.</p>
...
<script>
  $(document).ready(function() {
    ...
    $(document).on('click', '.verify', function() {
      ...
      $.ajax({
        url: "/moderator/contributedactverify/" + id,
        dataType: "json",
    ...
    $('#ajaxmodalform').on('submit', function(event) {
      ...
      $.ajax({
        url: "{{ route('moderator.contributedactupdate') }}",
        method: "post",
        data: $(this).serialize(),
        dataType: "json",
        success: function(data) {
    ...
    $('#moderator_remarks').on('change ', function(e) {
      ...
      if (!tested.test(testval)) {
        ...
        $('#moderator_remarkserror').slideDown("slow");
      } else {
        $('#moderator_remarkserror').hide();
