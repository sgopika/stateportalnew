...
<div class="row ">
 ...
 <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;  Communication Type List </strong></p>
   </div>
  <div class="col-12 py-1">
...
<div class="modal fade"  id="transactionmodal" tabindex="-1" role="dialog" aria-labelledby="addmodalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    ...
    <div class="row customformrow">
      ...
      <div class="col-md-6 py-2">
          <input type="text" class="form-control customform eng_xxxs fg-darkCrimson" maxlength="50" minlength="3" id="name" name="name" aria-describedby="HELPNAME" placeholder="Placeholder value">
...
$("#transactionbutton").click(function(event){
    ...
    $('#nameerror').hide();
    $('.modal-title').text('Add New CommunicationType');
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var action_url = '';
    if($('#action').val() == 'Add')
        action_url = "{{ route('admin.communicationtypestore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('admin.communicationtypeupdate') }}";
$('#loader').show();
    $.ajax({
         url: action_url,
         method:"post",
         ...
         { 
          //  alert(data.success);
            ...
            if(data.errors)
            {
              ...
              swal("Warning","Already an communication type with same name exists","warning");
              $('#loader').hide();
                      // window.location.reload();
              //  alert("Already an communication type with same name exists");
  ...
  $(document).on('click', '.edit', function(){
    $('#loader').show();
      ...
      $('#ajaxformresults').html('');
      $.ajax({
       url :"/admin/communicationtypeedit/"+id,
       dataType:"json",
       success:function(data)
        { 
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
    $.ajax({
      url:"/admin/communicationtypestatus/"+id,
      dataType:"json",
  ...
  $(document).on('click', '.delete', function(){
    ...
    swal(
		{
			title: "Deleting Communication Type",
			text: "Are you sure to Communication Type?",
			type: "warning",
    ...
    .then((isconfirm) => {
      ...
      $('#loader').hide();
			if (isconfirm) {
			  $.ajax({
            url:"/admin/communicationtypedestroy/"+element_id,
            dataType:"json",
            ...
            success:function(data)
            {
              ...
              swal({
                                                title:'Communication Type Deleted',
                                                text:data.success,
                                                type:'info',                                               
                                                buttonsStyling:false,
                                                reverseButtons:true
