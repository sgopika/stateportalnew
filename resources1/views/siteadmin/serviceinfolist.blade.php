...
<div class="row ">
  <div class="col-12 py-2  ">
   ...
   <div class="col-12 py-1">
     <p class="py-2"> <strong > <i class="fas fa-hand-point-right"></i> &nbsp;Service Info  </strong></p>
   </div>
  <div class="col-12 py-1">
        ...
        <div id="form_section">
         ...
         <div class="row customformrow">
            ...
            <div class="col-md-6 py-2">
                ...
                <p id="titleerror" style="display:none; color:#FF0000;" >A -Z a-z Characters,Numbers and special characters(like "",&()-$_=) allowed.</p>
            </div> <!-- ./col-md-6 -->
          </div> <!-- ./row -->
          <div class="row customformrow">
            <div class="col-md-6 py-2">
...
$('#ajaxmodalform').on('submit', function(event){ 
  ...
  if(usertype==3){
      ...
      if($('#action').val() == 'Add')
        action_url = "{{ route('siteadmin.serviceinfostore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('siteadmin.serviceinfoupdate') }}";

  } else if(usertype==4){

      if($('#action').val() == 'Add')
        action_url = "{{ route('webadmin.serviceinfostore') }}";

    if($('#action').val() == 'Edit')
        action_url = "{{ route('webadmin.serviceinfoupdate') }}";
         ...
         { 
            ...
            if(data.success)
            {
               ...
               swal("Updated Sucessfully")
                        .then((value) => {
                         $('#ajaxmodalform')[0].reset();
                         window.location.href='/siteadmin/serviceinfolist';
                        });
            }
         }
    });
  ...
  $(document).on('click', '.edit', function(){
      ...
      var usertype=$("#usertypeid").val(); 
      if(usertype==3){
         action_url2 = "/siteadmin/serviceinfoedit/"+id;
      } else if(usertype==4){
         action_url2 = "/webadmin/serviceinfoedit/"+id;
      }
      $.ajax({
       url :action_url2,
       dataType:"json",
   ...
   $(document).on('click', '.active', function(){
    ...
    var id = $(this).attr('id'); 
     ...
     var action_url3 = ''; 
      if(usertype==3){
         action_url3 = "/siteadmin/serviceinfostatus/"+id;
      } else if(usertype==4){
         action_url3 = "/webadmin/serviceinfostatus/"+id;
      }
    $.ajax({
      url:action_url3,
      dataType:"json",
  ...
  $('#ok_button').click(function(){
   var action_url2 = '';
    var usertype=$("#usertypeid").val(); 
      if(usertype==3){
         action_url2 = "/siteadmin/serviceinfodestroy/"+element_id;
      } else if(usertype==4){
         action_url2 = "/webadmin/serviceinfodestroy/"+element_id;
      }
    $.ajax({
            url:action_url2,
            dataType:"json",
