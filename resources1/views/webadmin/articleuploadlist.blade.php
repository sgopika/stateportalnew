...
<div class="container-fluid homepage adminpage">
  <div class="row ">
    ...
    <div class="col-12 py-1 px-2 ">
      <p class="eng_xxs px-3 fg-darkBrown"> Article Upload List </p>
    </div> <!-- ./col12 -->
          ...
          <tbody class="eng_xxxs">
            ...
            <tr>
              ...
              <td><span class="eng_xxxxs">
                  @if(substr($res->file,-4) == '.mp3')
                  <audio controls>
                    <source src="{{asset('Articleupload').'/'.$res->file}}" type="audio/ogg">
                  </audio>
                  @else
                  <img src="{{asset('Articleupload').'/'.$res->file}}" class="img-thumbnail displaythumbnail" alt="Logo" height="200" width="200">
                  @endif
                </span> </td>
...
<div class="modal fade" id="transactionmodal" tabindex="-1" role="dialog" aria-labelledby="addmodalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      @if(Auth::user()->usertypes_id==4)
      <form action="{{ route('webadmin.articleuploadstore') }}" id="ajaxmodalform" method="post" class="form-horizontal" enctype="multipart/form-data">
        @elseif(Auth::user()->usertypes_id==5)
        <form action="{{ route('editor.articleuploadstore') }}" id="ajaxmodalform" method="post" class="form-horizontal" enctype="multipart/form-data">
          @elseif(Auth::user()->usertypes_id==6)
          <form action="{{ route('photoeditor.articleuploadstore') }}" id="ajaxmodalform" method="post" class="form-horizontal" enctype="multipart/form-data">
            @endif
            @csrf
            <div class="modal-body adminpage">
              ...
              <div id="form_section">
                ...
                <div class="row customformrow">
                  ...
                  <div class="col-md-6 py-2">
                    <select class="form-control customform eng_xxxs fg-darkCrimson" id="articles_id" name="articles_id" required></select>
                  </div> <!-- ./col-md-6 -->
                </div> <!-- ./row -->
                <div class="row customformrow">
                  <div class="col-md-6 py-2">
                ...
                <div class="row customformrow">
                  ...
                  <div class="col-md-6 py-2">
                    ...
                    <p id="durationerror" style="display:none; color:#FF0000;">numbers are allowed.</p>
                  </div> <!-- ./col-md-6 -->
                </div> <!-- ./row -->
...
<div class="modal fade" id="transactionmodal1" tabindex="-1" role="dialog" aria-labelledby="addmodalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      @if(Auth::user()->usertypes_id==4)
      <form action="{{ route('webadmin.articleuploadupdate') }}" id="ajaxmodalform1" method="post" class="form-horizontal" enctype="multipart/form-data">
        @elseif(Auth::user()->usertypes_id==5)
        <form action="{{ route('editor.articleuploadupdate') }}" id="ajaxmodalform1" method="post" class="form-horizontal" enctype="multipart/form-data">
          @elseif(Auth::user()->usertypes_id==6)
          <form action="{{ route('photoeditor.articleuploadupdate') }}" id="ajaxmodalform1" method="post" class="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="modal-body adminpage">
              ...
              <div id="form_section">
                <div class="row customformrow">
                  ...
                  <div class="col-md-6 py-2">
                    <select class="form-control customform eng_xxxs fg-darkCrimson" id="articles_id1" name="articles_id1" required></select>
                  </div> <!-- ./col-md-6 -->
                </div> <!-- ./row -->
                <div class="row customformrow">
                  <div class="col-md-6 py-2">
                ...
                <div class="row customformrow">
                  ...
                  <div class="col-md-6 py-2">
                    ...
                    <p id="durationerror1" style="display:none; color:#FF0000;">Only A -Z a-z Characters, period and numbers are allowed.</p>
                  </div> <!-- ./col-md-6 -->
                </div> <!-- ./row -->
...
<script>
  ...
  // console.log('summernote\'s content is chandged.');
        ...
        var charRegExp  = /[@#$%^*()?!":{}|]/;
        var textareaValue1 = $('#enabout').summernote('code').replace(/(<([^>]+)>)/ig, "");
        if (charRegExp.test(textareaValue1)) {  
          $('#enabout').summernote('undo');
    ...
    $("#transactionbutton").click(function(event) {
      ...
      $('.modal-title').text('Add New Article upload');
      ...
      var action_url = '';
      if (usertype == 4) {
        action_url = "{{ route('webadmin.articleuploadcreate') }}";
      } else if (usertype == 5) {
        action_url = "{{ route('editor.articleuploadcreate') }}";
      } else if (usertype == 6) {
        action_url = "{{ route('photoeditor.articleuploadcreate') }}";
      }
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $('#articles_id').empty();
          $('#articles_id').append($('<option></option>').val('').html('Select'));
    ...
    $(document).on('change', '#contenttypes_id', function() {
      ...
      var action_url2 = '';
      if (usertype == 4) {
        action_url2 = "/webadmin/articleuploadfiletypelist/" + id;
      } else if (usertype == 5) {
        action_url2 = "/editor/articleuploadfiletypelist/" + id;
      } else if (usertype == 6) {
        action_url2 = "/photoeditor/articleuploadfiletypelist/" + id;
      }
      if (id != '') {
        $.ajax({
          url: action_url2,
    ...
    $(document).on('change', '#contenttypes_id1', function() {
      ...
      var action_url3 = '';
      if (usertype == 4) {
        action_url3 = "/webadmin/articleuploadfiletypelist/" + id;
      } else if (usertype == 5) {
        action_url3 = "/editor/articleuploadfiletypelist/" + id;
      } else if (usertype == 6) {
        action_url3 = "/photoeditor/articleuploadfiletypelist/" + id;
      }
      if (id != '') {
        $.ajax({
          url: action_url3,
    ...
    $(document).on('click', '.edit', function() {
      ...
      var action_url4 = '';
      if (usertype == 4) {
        action_url4 = "/webadmin/articleuploadedit/" + id;
      } else if (usertype == 5) {
        action_url4 = "/editor/articleuploadedit/" + id;
      } else if (usertype == 6) {
        action_url4 = "/photoeditor/articleuploadedit/" + id;
      }
      $.ajax({
        url: action_url4,
        dataType: "json",
        success: function(data) {
          ...
          $("#uploadedimage").attr('src', "{{asset('Articleupload')}}/" + data.resultdata.file);
          ...
          $('#articles_id1').empty();
          $('#articles_id1').append($('<option></option>').val('').html('Select'));
          $.each(data.article, function(index, element) {
            $('#articles_id1').append(
              $('<option></option>').val(element.id).html(element.entitle)
            );
            element.id == data.resultdata.articles_id ? $('#articles_id1').val(element.id).prop('selected', true) : '';
          });
    ...
    $(document).on('click', '.active', function() {
      ...
      var action_url5 = '';
      if (usertype == 4) {
        action_url5 = "/webadmin/articleuploadstatus/" + id;
      } else if (usertype == 5) {
        action_url5 = "/editor/articleuploadstatus/" + id;
      } else if (usertype == 6) {
        action_url5 = "/photoeditor/articleuploadstatus/" + id;
      }
      $.ajax({
        url: action_url5,
        dataType: "json",
    ...
    $(document).on('click', '.delete', function() {
      ...
      var action_url6 = '';
      if (usertype == 4) {
        action_url6 = "/webadmin/articleuploaddestroy/" + element_id;
      } else if (usertype == 5) {
        action_url6 = "/editor/articleuploaddestroy/" + element_id;
      } else if (usertype == 6) {
        action_url6 = "/photoeditor/articleuploaddestroy/" + element_id;
      }

      swal(
            {
              title: "Deleting Article Upload List",
              text: "Are you sure to delete Article Upload List?",
              type: "warning",
              buttons: true,
              dangerMode: true
            })
            ...
            if (isconfirm) {
              $.ajax({
                  ...
                  success:function(data)
                  {
                    $('#loader').hide();
                      swal({
                                                        title:'Article Upload List Deleted',
                                                        text:data.success,
                                                        type:'info',                                               
