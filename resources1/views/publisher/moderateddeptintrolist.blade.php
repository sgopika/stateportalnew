    ...
    <nav aria-label="breadcrumb" class="breadcrumbinner py-1 eng_xxxs">
      <ol class="breadcrumb justify-content-end px-3 pt-2">
        ...
        <li class="breadcrumb-item"><a class="no_link" href="{{ route('publisherhome') }}"><i class="fas fa-home"></i>&nbsp;Home</a></li>
         <li class="breadcrumb-item"><a class="no_link" href="{{ route('publisher.moderateditems', $val) }}"> Items</a></li>
            ...
            <tbody class="eng_xxxs">
               @php
                ...
                <tr>
                  ...
                  @if($val==1)
                  <td>
                   @if($res->approve_remarks!='')
                  <span class="eng_xxxxs"> {{ $res->approve_remarks }}</span><hr class="py-1">
                  @endif
                  <div class="btn-group" role="group" aria-label="Actionbuttons">
              
              <button type="button" class="edit btn btn-sm bg-lightBrown fg-darkCrimson eng_xxxxs approve" name="approve" id="{{ $res->id }}" data-toggle="tooltip" data-placement="top" title="Approve"> <i class="fas fa-pencil-alt"></i>&nbsp;Approve</button>&nbsp; &nbsp;
...
<div class="modal fade"  id="transactionmodal" tabindex="-1" role="dialog" aria-labelledby="addmodalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm" role="document">
    ...
    <div class="row customformrow">
      ...
      <div class="col-md-12 py-2">
          <textarea class="form-control customform eng_xxxs fg-darkCrimson" id="approve_remarks" name="approve_remarks" aria-describedby="HELPNAME" placeholder="Remarks" minlength="3" maxlength="1000" required></textarea>
      </div> <!-- ./col-md-6 -->
      <p id="approve_remarkserror" style="display:none; color:#FF0000;">Only Alphabets,dot and , are allowed.</p>
...
$(document).on('click', '.approve', function(){
      ...
      $.ajax({
       url :"/publisher/moderateddeptintroapprove/"+id,
       dataType:"json",
  ...
  $('#ajaxmodalform').on('submit', function(event){ 
    ...
    $.ajax({
         url: "{{ route('publisher.moderateddeptintroupdate') }}",
         method:"post",
         data:$(this).serialize(),
         dataType:"json",
         success:function(data)
  ...
  $('#approve_remarks').on('change ', function(e) {
      var testval = this.value;
      var tested = new RegExp('^[a-zA-Z \s\.\,]+$');
      if (!tested.test(testval)) {
        $('#approve_remarks').val('');
        $('#approve_remarkserror').slideDown("slow");
      } else {
        $('#approve_remarkserror').hide();
