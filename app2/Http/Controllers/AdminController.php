...
//                $number = $mobile;
    ...
    public function usertypeupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Usertype::where([['id','!=',$request->hidden_id],['entitle','=',$request->name]])->exists() ? 1 : 0;
            if($chkrows==0){
	            $form_data = array(
	                'entitle'    =>  $request->name
	            );
    ...
    public function componentupdate(Request $request)
    {
        ...
        // if ($valErr->fails()) {
        //     return response()->json(['errors' => $valErr]);
        // }else{
        //     return response()->json(['errors' => 'xx'.$valErr]);
        // }
        if($request->ajax())
        {
             $userid=Auth::user()->id;

        $chkrows= Component::where([['id','!=',$request->hidden_id],['entitle','=',$request->name]])->exists() ? 1 : 0;
            if($chkrows==0){
    ...
    public function menulinktypeupdate(Request $request)
    {
        ...
        {
            $userid=Auth::user()->id;
            $chkrows= Menulinktype::where([['id','!=',$request->hidden_id],['entitle','=',$request->name]])->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'    =>  $request->name,
                    'users_id' => $userid
    ...
    public function contenttypestore(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Contenttype::where([['id','!=',$request->hidden_id],['entitle','=',$request->name],['maltitle','=',$request->malname]])->exists() ? 1 : 0;
            if($chkrows==0){
    ...
    public function contenttypeupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Contenttype::where([['id','!=',$request->hidden_id],['entitle','=',$request->name],['maltitle','=',$request->name]])->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'    =>  $request->name,
                    'maltitle'   =>  $request->malname,
    ...
    public function filetypeupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Filetype::where([['id','!=',$request->hidden_id],['entitle','=',$request->name],['maltitle','=',$request->malname],])->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'        =>  $request->name,
                    'maltitle'        =>  $request->malname,
    ...
    public function communicationtypeupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Communicationtype::where([['id','!=',$request->hidden_id],['entitle','=',$request->name]])->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'    =>  $request->name,
                    'users_id'       => Auth::user()->id
    ...
    public function userupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= User::where([['id','!=',$request->hidden_id],['name','=',$request->name]])
            ->orWhere([['id','!=',$request->hidden_id],['email','=',$request->email]])
            ->exists() ? 1 : 0;
            // return response()->json(['errors' => $chkrows]);
            if($chkrows==0){
                $form_data = array(
                    'name'          =>  $request->name,
                    'email'         => $request->email,
                ...
                return response()->json(['errors' => 'Already an User with Name/Email exists.']);
            }    
        }
...
public function sitesettinglist(Request $request)
    ...
    {
        $request->validate([
            'name'  =>'required|min:3|max:300|regex:/(^[A-Za-z\s ]+$)/'
    ...
    {
        $request->validate([
            'name'  =>'required|min:3|max:500|regex:/(^[-0-9A-Za-z\s ]+$)/'

        ]);
        if($request->ajax())
        {
            $chkrows= Sitesetting::where([['id','!=',$request->hidden_id],['entitle','=',$request->name]])->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'    =>  $request->name,
                    'users_id'          => Auth::user()->id
...
public function languagelist(Request $request)
    ...
    public function languageupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Language::where([['id','!=',$request->hidden_id],['entitle','=',$request->name]])->exists() ? 1 : 0;
            if($chkrows==0){
	            $form_data = array(
	                'entitle'    =>  $request->name,
                    'users_id'          => Auth::user()->id
    ...
    {
        $request->validate([
            'name'  =>'required|min:3|max:1000|regex:/(^[A-Za-z.,&\s ]+$)/',
            'malname'   =>'required|min:3|max:1000|regex:/(^[\P{Malayalam}_.,&]+$)/'
        ]);
        if($request->ajax())
        {
            ...
            $chkrows= Sitecontrollabel::where([['id','!=',$request->hidden_id],['entitle','=',$request->name],['maltitle','=',$request->malname]])->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'    =>  $request->name,
                    'maltitle'   =>  $request->malname,
    ...
    public function deptcategoryupdate(Request $request)
    {
        ...
        if($request->ajax())
        {
            $chkrows= Deptcategory::where([['id','!=',$request->hidden_id],['entitle','=',$request->name],['maltitle','=',$request->maltitle]])->exists() ? 1 : 0;
            if($chkrows==0){
                $form_data = array(
                    'entitle'        =>  $request->entitle,
                    'maltitle'        =>  $request->maltitle,
